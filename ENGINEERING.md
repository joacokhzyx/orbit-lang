...
| STAB-2 | Self-host | Remove the dual-compiler drift: the Zig frontend (`src/main.zig`) and the self-hosted `.orb` pipeline currently emit different C for the same source. Make the `.orb` pipeline the single source of truth (Zig becomes a thin loader). | Same input → byte-identical `temp_build.c`/`orbit_selfhost_build.c` regardless of which driver produced it. | 🔍 Measurement (2026-08-16): the seed (`stage3.exe`, steel backend) **cannot compile route programs at all** — see PARITY-0..3 below. Until those are fixed there is no C to diff, so byte-identity is blocked upstream. |
...
| PARITY-0 | `compiler/parser.orb` | `parseRouteDecl` (541) | The self-host seed **segfaults (0xC0000005) on the canonical route syntax** `route GET "/p"`: `consume(parser, TokType.StringLiteral, …)` for the HTTP method rejects the bare `GET` identifier, and the resulting error-recovery path leaves the parser state corrupt (garbage `Token.line`/`col` seen in unrelated diagnostics, e.g. `line -467998816`). Parse of the workaround form `route "GET" "/p"` succeeds, proving the token-level corruption is triggered by the error path, not by route parsing per se. `parseProgram` never reached sema; `orbit_selfhost_build.c` is not written. Blocks STAB-2 entirely. | **High** |
| PARITY-1 | `compiler/optimizer.orb` | — | Even with the parser workaround (`route "GET" "/p"`), the seed fails **inside `optimizeIRModule`** after `buildAST` reports `funcs=1` (exit 1, no C written). Second route-parsing/optimizer blocker for STAB-2. | **High** |
...
| PARITY-3 | `compiler/*.orb` | — | Observed from the STAB-2 drift probe: the seed only ever compiles `compiler/main.orb` in the fixed-point (221 functions), so no `route`-bearing program exercises the self-host parser/builder/optimizer/codegen. The 3-stage byte-identity proves self-consistency, NOT feature coverage — route programs are a blind spot. Fixing PARITY-0/1 requires regenerating the fixed-point canonical via `scripts/verify_seed.py --bootstrap`. | — |
