...
| COMPILE-0 | `benchmarks/marketing_suite/servers/02_auth_server.orb` | — | Does not compile. Codegen: `no member named 'body' in 'struct OrbitModel'` — request model field access emitted against the wrong C struct. | **High** |
| COMPILE-1 | `benchmarks/marketing_suite/servers/03_page_cache_server.orb` | — | Does not compile. Parser: `unexpected token 'TypeSet'` — syntax rejected at parse time, pre-existing. | **High** |
| COMPILE-2 | `examples/catalog_service.orb` | — | Does not compile. Codegen builtin arity mismatch on `orbit_http_query_get`, `orbit_db_query_where`, `orbit_http_body_get`. | **Medium** |
