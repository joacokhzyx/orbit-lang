...
| CODE-1 | `compiler/sema.orb` | `reservedCFunctionName` | ~~A user function named `read`, `write`, `open` or `close` collided with the C library declaration of the same name and failed at link time with "conflicting types", even though `orbit check` passed.~~ Partially resolved (2026-09-26): the front end now refuses the name with a diagnostic that names the collision and suggests a rename, so the failure is an Orbit error at check time instead of a C toolchain message at link time. The list is deliberately narrow: only symbols that are both plausible user names and genuinely declared in the header set a generated program pulls in (`<stdio.h>`, `<stdlib.h>`, `<string.h>`, `<stdint.h>`, plus `unistd.h`/`sys/socket.h` on POSIX and `winsock2.h` on Windows). The real fix is still open: user symbols should be mangled at emission so no name can collide at all, which touches every call site in every program and is why it was not done here. | **Medium** |
| DX-0 | `compiler/pipeline.orb` | `checkSourceWithFile` / `compileSource` | `orbit check` reports semantic errors on stdout while `orbit build` reports the same error on stderr (pipeline.orb:122 uses `print`, :264 uses `orbit_os_write_stderr_selfhost`). Every other gate in the repo follows the convention that failures go to stderr, so a script reading only stderr sees a clean `check`. Not changed here because the fix is a behaviour change for anyone parsing that output, and it belongs with a decision about the output contract rather than inside a name check. | **Low** |
...
| FMT-1 | `runtime/arena.c`, `runtime/selfhost.c` | - | The runtime's out-of-memory path returns NULL or empty rather than failing loudly, and `orbit_string` is a NUL-terminated `const char*` everywhere, so an allocation failure and a legitimately empty string are indistinguishable at most boundaries. That is what let FMT-0 be silent. Fixing it means distinguishing the two at the arena boundary, which touches every `orbit_alloc` caller; `orbit_file_write` no longer depends on it, so this is hardening rather than an open data-loss path. | **Medium** |
