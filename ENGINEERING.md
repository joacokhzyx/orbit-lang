...
The repository currently uses the self-hosted Orbit compiler and committed canonical C seed described in [docs/architecture/SOVEREIGNTY.md](docs/architecture/SOVEREIGNTY.md). References to the removed Zig seed or historical `src/` paths are retained only when they explain a migration, validation lineage, or compatibility decision; they are not current build requirements.
...
Steps:
1. Concatenate `compiler/selfhost/stage3.exe.c` with `runtime/*.c`, inlining all `#include` references to produce a single self-contained translation unit.
   - Implemented by `scripts/amalgamate.py`: recursively inlines every project-relative `#include "..."` (resolved from the including file's directory, fallback to `runtime`), wraps each inlined file in its own per-path include guard so an `#ifdef`-gated occurrence cannot mask a later unconditional one, and leaves system includes untouched. Output is deterministic (fixed order, LF newlines).
...
| STAB-9 | SemA/Codegen | Result-value handling gaps found during Wave 1 std work: (1) try/catch on a result-typed parameter miscompiles (aggregate-value C error) while inline try on the call works; (2) `return ok(int-expr)` directly is rejected ("ok body must be a string") while assign-through-typed-binding works; (3) binding a result-returning call to an untyped val miscompiles (annotate `val x: result` or use inline try); (4) the `.to_int()` method emits a call to a nonexistent C helper (use extern `orbit_string_to_int`). | Helpers over results (isOk/unwrapOr/assertErr) compile, run, and read correctly; `return ok(expr)` accepted for any typed expr; untyped val binding of result calls infers result; `.to_int()` links or is removed. | Open (2026-09-24): found via tests/std Wave 1 probes; workarounds documented in std/string/string.orb and tests/std/test_string.orb. |
...
| ~~ARENA-0~~ | `runtime/test_arena.c` | - | ~~No cross-request arena isolation test. Now added `test_cross_request_sequential_isolation` and `test_cross_request_concurrent_isolation`; 27/27 tests pass. (2026-08-16)~~ | ✅ Resolved |
...
| ~~PARITY-2~~ | `compiler/lexer.orb` | `initLexer` | ~~The self-host lexer rejected the UTF-8 BOM (`EF BB BF`) as `Lexer error: invalid character (code 239/187/191)`. `initLexer` now skips the BOM before tokenization, with `tests/suite/bom_utf8.orb` covering the regression. Verified with fixed point `2/2`, parity `27/27`, and behavior suite `14/14` using w64devkit GCC 16.2.0 (2026-09-10).~~ | ✅ Resolved |
...
| PARITY-5 | `compiler/builder.orb`, `compiler/c_backend.orb`, `compiler/pipeline.orb` | In progress (2026-08-17): the selfhost builder now registers the frontend runtime return-type table; lowers route `req.query/body/json/param/header/bearer_token/role/has_role/file` calls with the frontend's implicit `req`/`req->headers` operands; lowers bare `req.body/json/bearer_token/role`; and lowers uppercase static model `all/where/find/create/delete` calls with a lowercase plural constant table operand. The selfhost C backend now injects `arena` for the matching HTTP/DB/auth runtime calls and emits these call arguments/results without pointer casts. Fresh combined probe C contains the expected runtime calls. Remaining: (1) add selfhost DB-use detection so ORM programs compile/link with `ORBIT_WITH_DB` and the bundled SQLite import library, and mirror DB init/close in the route server; (2) close byte-level C drift exposed by the combined probe: route versus non-route `always_inline` placement, constant direct HTTP-call arguments, route-local declaration/cast policy, and generated jump-label spelling; (3) run individual `r6`, `c3`, `c4`, and `stab2` oracle comparisons, then the complete probe matrix; (4) rerun `verify_seed.py --bootstrap` to completion and record canonical/chain hashes, then run `zig build test --summary all`. Quoted route methods remain intentionally rejected by both lineages. The full `body.id` stretch behavior is not yet ported or validated. | 🚧 In progress |
