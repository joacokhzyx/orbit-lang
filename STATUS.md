...
| :--- | :--- | :--- |
| **Host Compiler (`orbit.exe`)** | **Production Ready** | Built via Zig 0.17.0-dev (local `0.17.0-dev.1503+1f1bee62e`; CI pinned to `0.17.0-dev.1737+de207594e`, gated by a `zig version` check); supports dual backends (`--backend=c`, `--backend=native`). |
...
| **Self-Hosted Compiler (`compiler/`)** | **Converged** | Written 100% in Orbit across 19 modules (16 top-level `.orb` + 3 in `compiler/frontend/`). Includes AST parser, IR builder, C backend, and recursive module resolver (`resolver.orb`). The bootstrap now reaches a fixed point: stage1→stage2→stage3→stage4 all succeed and `stage3.exe.c` is byte-identical to `stage4.exe.c` (see `HANDOFF-selfhost.md`). |
...
- **Pillar 5**: Self-hosting bootstrap pipeline implemented (`orbit bootstrap`). The fixed point is reached: stage1→stage2→stage3→stage4 converge and `stage3.exe.c` is byte-identical to `stage4.exe.c`, so self-hosting is verified. See `HANDOFF-selfhost.md`.
