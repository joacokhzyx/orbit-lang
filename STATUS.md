...
| **Self-Hosted Compiler (`compiler/`)** | **In progress** | Written 100% in Orbit across 19 modules (16 top-level `.orb` + 3 in `compiler/frontend/`). Includes AST parser, IR builder, C backend, and recursive module resolver (`resolver.orb`). Fresh bootstrap run 2026-08-13: stage1→stage2 succeeds, but stage2→stage3 still crashes (`0xC0000005`) in the C-backend pass while building `tokTypeToString` (see `HANDOFF-selfhost.md` §2). |
...
- **Pillar 5**: Self-hosting bootstrap pipeline implemented (`orbit bootstrap`). The fixed point is not yet reached: stage2→stage3 does not converge (crashes in the C-backend pass at `tokTypeToString`), so `zig build test` cannot claim 100% self-hosting verification.
